...
const float minTempMor = -18;                 // Минимальная температура морозилки
...
// **************************
void ispProvTemp () {
   for (int i = 0; i <= meta; i++ ) { // ?? Вынести в отдельную функцию, 6 сделать переменной ??? int meta = 6; i <= meta
    ispthermProv ();
    Serial.println (tisp);
    delay(30000);
  } 
...
void cycleMorMin() { 
  // если тмор больше 0 и емагн увеличивается, тогда - клапан
  // иначе вкл. цикл фена и ждём температуру 0 градусов после чего отключаем фен
  // и ждем температуру испарителя -24 градуса 
